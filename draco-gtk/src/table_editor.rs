...
impl EditableColumnRow {
    fn new(existing: Option<&TableDetailColumn>) -> Self {
        ...
        let data_type = gtk::Entry::builder().placeholder_text("type (e.g. text, integer)").width_chars(18).build();
        ...
        if let Some(c) = existing {
            ...
            if c.is_primary_key {
                root.append(&gtk::Label::builder().label("PK").css_classes(["dim-label"]).build());
            }
        }
        ...
        Self { root, original_name: existing.map(|c| c.name.clone()), name, data_type, nullable, default, removed: RefCell::new(false), remove_btn }
    }
    ...
    fn set_removed_style(&self, removed: bool) {
        ...
        self.remove_btn.set_icon_name(if removed { "edit-undo-symbolic" } else { "user-trash-symbolic" });
    }
    ...
    fn to_edit(&self) -> ColumnEdit {
        ...
        ColumnEdit {
            ...
            default: if default.trim().is_empty() { None } else { Some(default) },
...
pub fn open(
    ...
    let table_name_row = adw::EntryRow::builder().title("Table name").text(table.as_str()).build();
    ...
    let scroller = gtk::ScrolledWindow::builder().child(&content).vexpand(true).min_content_width(620).build();
    ...
    let dialog = adw::Dialog::builder().title(format!("Edit Table · {schema}.{table}")).content_width(660).content_height(700).build();
    let toolbar_view = adw::ToolbarView::new();
    let header = adw::HeaderBar::new();
    let apply_btn = gtk::Button::builder().label("Apply").css_classes(["suggested-action"]).build();
    ...
    let build_statements = {
        ...
        move |table_name_row: &adw::EntryRow| -> Result<Vec<String>, String> {
            ...
            Ok(queries::build_alter_table_statements(&schema, &table, new_table_name.trim(), &original_columns, &edits))
        }
    };
