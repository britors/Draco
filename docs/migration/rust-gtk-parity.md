...
| M3 | Editor SQL | Tauri: textarea local com formatação básica, seletor de conexão, seleção ou buffer completo, execução e cancelamento por operação, abas múltiplas numeradas, atalhos `F8`/`Ctrl+Enter` (rodar), `F10` (`EXPLAIN` puro em JSON, nunca `ANALYZE`) e `Ctrl+Shift+S` (snippet); GTK: GtkSourceView5 | implementado (falta no Tauri: syntax highlighting e autocomplete baseado no schema) |
| M3 | Resultados | Tauri: grid de colunas dinâmicas com limite de 500 linhas renderizadas, detalhe de linha acessível, cópia TSV e export CSV/JSON testados com delimitadores, quebras de linha, `NULL` e valores estruturados; GTK: `ColumnView` | implementado (falta: virtualização real) |
